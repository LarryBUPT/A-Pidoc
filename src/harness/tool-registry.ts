...
import type { AgentTask, HarnessTool } from "./contracts.js";
...
export class ToolRegistry {
  ...
  toPiTools(task: AgentTask): AgentTool[] {
    return [...this.tools.values()].filter(t => task.allowedToolBundles.includes(t.bundle)).map(tool => ({
      ...
      execute: async (_id, args, signal) => {
        ...
        try {
          const result = await tool.execute(args, { runId: task.id, environment: task.environment, ...(signal ? { signal } : {}) });
