...
export async function runHarnessCommand(mode:string,options:Map<string,string>):Promise<void> {
  ...
  try {
    ...
    let s;
    if(mode==="agent-run") {
      const task:AgentTask={id:randomUUID(),goal:sandbox?`Investigate the failing POST ${sandbox.endpoint} request with Content-Type text/plain and amount 42. Determine the cause and validate a corrected request with actual HTTP evidence.`:"Complete a supported isolated migration for the registered contracts/client, investigate actual diff/impact, request approval through the intended isolated tool call, reissue exactly after approval, and verify with actual regression tests.",taskFamily:profile!,environment:"sandbox",inputArtifacts:[],allowedToolBundles:[profile!,"shared"],risk:sandbox?"low":"high",budget:{maxModelCalls:20,maxToolCalls:40,maxTokens:80_000,maxCostUsd:1,maxDurationMs:180_000}};
