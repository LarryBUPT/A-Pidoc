...
export async function applyMigrationPatch(root: string, plan: MigrationPatchPlan): Promise<void> { const file = safeFile(root, plan.file); const source = await readFile(file, "utf8"); const count = source.split(plan.before).length - 1; if (count !== 1) throw new Error(`Migration patch expected one occurrence in ${plan.file}, found ${count}`); await writeFile(file, source.replace(plan.before, plan.after), "utf8"); }
...
export function testContent(title: string, body: Record<string, unknown> | null, changes: ContractChange[]): string {
...
export async function runTests(root: string, files: string[], timeoutMs: number): Promise<RepositoryTestRun> {
  const started = Date.now(); const args = ["--test", "--test-reporter=tap", ...files.map((file) => safeFile(root, file))];
