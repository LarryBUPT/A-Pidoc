...
本文主体描述 V4 已实现并由测试覆盖的代码；新增 V4.5 运行带见文末迁移入口与 [Pi RFC](pi-0.85.1-rfc.md)。平台适配器解析固定载荷并构造回复请求，冻结评测使用内存 Connector；真实企业账号、生产 Webhook、异步队列、任意 AST/动态数据流、向量知识库、前端界面和生产部署不在当前架构中。
