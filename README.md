...
| 想在常用 AI 开发工具中复用诊断能力 | 通过 MCP 接入受控的文档读取、请求执行与结果核对 |
| 想用仓库外样本检验能力 | 运行可选的外部 API 基准，并保留可复查的结果 |
...
| 运行受限的外部 API 兼容性与配对评测 | [外部 API 基准指南](docs/guides/external-api-benchmark.md) |
...
项目用可重复的本地检查核对已有能力；需要联网、容器或真实模型的实验会单独启用，不会混入默认演示。想了解结果是怎样验证的，以及每类结果能说明什么，可以查看[验证指南](docs/verification.md)。

## 🛡️ 使用前了解

API Doctor 当前是一个提供命令行、本地 HTTP 服务与 MCP 接入的实验性项目，适合学习、接口联调和受控环境中的验证。
...
- **MCP 目前用于本地接入。** 宿主需要预先指定接口文档和允许访问的目标；它不会绕过原有安全限制，也不提供任意写操作。
- **外部验证需要单独开启。** 联网、容器和付费模型不会随默认流程自动运行；仓库提供实验入口，不等于所有外部场景都已经验证。
...
| 如何接入 MCP | [MCP Server 使用指南](docs/guides/mcp-server.md) |
| 如何运行外部 API 基准 | [外部 API 基准指南](docs/guides/external-api-benchmark.md) |
