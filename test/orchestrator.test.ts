...
test("all controlled cases complete the diagnosis and verification loop", async () => {
  ...
  for (const item of cases) {
    const report = await app.run(structuredClone(item));
...
test("a bad host is blocked before the HTTP tool runs", async () => {
  ...
  const report = await new DebugOrchestrator(new FixtureHttpTool(), new DeterministicReasoner()).run(item);
...
test("the trace uses monotonic sequence numbers", async () => {
  const report = await createFixtureApp().run(getCase("body-type"));
  assert.deepEqual(
