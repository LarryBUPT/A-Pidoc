...
for(const family of ["runtime-api","repository-contract"]) {
  ...
  try {
    ...
    const task={id:randomUUID(),goal:sandbox?`Investigate the failing POST ${sandbox.endpoint} request with Content-Type text/plain and amount 42. Determine the cause and validate a corrected request using actual HTTP evidence.`:"Assess the registered previous/next contract change and client compatibility. Propose a supported safe migration, await exact approval before isolated changes, then verify with actual regression tests. Cite actual diff/patch/test evidence.",taskFamily:family,environment:"sandbox",inputArtifacts:[],allowedToolBundles:[family,"shared"],risk:sandbox?"low":"high",budget:{maxModelCalls:20,maxToolCalls:40,maxTokens:80000,maxCostUsd:1,maxDurationMs:180000}};
